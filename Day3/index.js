function plotWire(wirePath){
    let newArr = [[0,0]];
    for (let direction of wirePath){
        if (direction.slice(0,1) == 'R'){
            for (let i = 0; i < direction.slice(1); i++){
                newArr.push([newArr[newArr.length-1][0] + 1 , newArr[newArr.length-1][1]]);
            }
            ...
            for (let i = 0; i < direction.slice(1); i++){
                newArr.push([newArr[newArr.length-1][0], newArr[newArr.length-1][1] - 1]);
            }
...
function filterCommon(pathA, pathB){
    newArr = [];
    for (let x of pathB){
        for(let y of pathA){
            if (x[0] === y[0] && x[1] === y[1])
